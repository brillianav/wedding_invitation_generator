...
if (!class_exists('Brilli_Wedding_Invitation_Maker')) {
    ...
    class Brilli_Wedding_Invitation_Maker {
        ...
        public function register_hooks() {
            ...
            add_action('init', array($this, 'remove_legacy_history_data'));
        ...
        /**
         * Remove data created by the discontinued generation-history feature.
         */
        public function remove_legacy_history_data() {
            global $wpdb;

            if (false === get_option(self::LEGACY_HISTORY_VERSION_OPTION, false)) {
                return;
            }

            $legacy_table = $wpdb->prefix . 'brilli_wim_history';
            $removed = $wpdb->query("DROP TABLE IF EXISTS {$legacy_table}"); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Table name uses the trusted WordPress prefix.

            if (false !== $removed) {
                delete_option(self::LEGACY_HISTORY_VERSION_OPTION);
            }
        }
